...
function getFileContentFromCommit(filePath, commitRef) {
  ...
  try {
    ...
    console.warn(
      `Warning: Could not find file '${filePath}' at commit '${commitRef}'. Assuming it's a new file.`,
    );
    return "";
  }
}
  ...
  for (const key in targetObj) {
    if (Object.prototype.hasOwnProperty.call(targetObj, key)) {
      if (!Object.prototype.hasOwnProperty.call(sourceObj, key)) {
        console.log(
          `- Deleting obsolete key '${key}' from target translations.`,
        );
...
async function main() {
  ...
  if (!currentCommitSha) {
    console.error(
      "Error: GITHUB_SHA environment variable is not set. This script is intended to be run in a GitHub Action.",
    );
    process.exit(1);
  }
  ...
  if (fs.existsSync(STATE_FILE_PATH)) {
    try {
      ...
      if (state.lastTranslatedCommit) {
        ...
        console.log(
          `State file found, but 'lastTranslatedCommit' is null. This will be treated as the first run.`,
        );
      }
    } catch (e) {
      console.error(
        `Error reading or parsing ${STATE_FILE_PATH}. Treating as first run.`,
        e,
      );
    }
  } else {
    console.log(
      `'${STATE_FILE_PATH}' not found. This must be the first run. All strings will be translated.`,
    );
  }
  ...
  for (const locale of LOCALE_CONFIGS) {
    ...
    if (!localeFileExists) {
      console.log(
        `Target file ${targetYamlPath} does not exist. It will be fully translated from the current source.`,
      );
      ...
      if (keysToTranslate.length > 0) {
        console.log(
          `Translating ${keysToTranslate.length} new/modified strings...`,
        );
  ...
  if (anyFileWasModified || isFirstRun) {
    ...
    console.log(
      `'${STATE_FILE_PATH}' has been updated. Please ensure this file is committed.`,
    );
  } else {
    console.log(
